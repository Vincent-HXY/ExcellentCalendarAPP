...
bool jstring_to_utf8(
  ...
  if (chars == nullptr) {
    ...
    reason = "JNI could not read the Search request string";
    return false;
  }
  output.clear();
  ...
  if (!valid) {
    output.clear();
    reason = "Search request contains malformed UTF-16";
  }
  return valid;
}
...
jstring emergency_failure_result(JNIEnv* env) noexcept {
  constexpr const char* fallback =
      R"({"contract_version":2,"data":null,"error":{"code":"NATIVE_INTERNAL_ERROR","details":{"reason":"Search JNI failure serialization failed"},"message":"Native internal error","retryable":false},"ok":false,"request_id":"00000000-0000-4000-8000-000000000000"})";
  if (env->ExceptionCheck()) env->ExceptionClear();
...
jstring call_search_boundary(
  ...
  try {
    ...
    return failure_result(
        ...
        "Search response is not valid UTF-8");
...
Java_com_excellentcalendar_excellent_1calendar_bridge_native_JniNativeCalendarCoreBridge_nativeQuerySearchV2(
    JNIEnv* env,
    jobject /* this */,
    jstring request_json) {
  return call_search_boundary(
      env, request_json, excellent_calendar::boundary::api::search_query_v2);
}
