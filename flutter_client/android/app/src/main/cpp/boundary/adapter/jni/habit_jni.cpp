...
bool jstring_to_utf8(
  ...
  if (chars == nullptr) {
    ...
    reason = "JNI could not read the Habit request string";
    return false;
  }
  output.clear();
  ...
  if (!valid) {
    output.clear();
    reason = "Habit request contains malformed UTF-16";
  }
  return valid;
}
...
jstring utf8_to_jstring(JNIEnv* env, std::string_view input) {
  ...
  return env->NewString(
      output.empty() ? nullptr : output.data(), static_cast<jsize>(output.size()));
}

jstring emergency_failure_result(JNIEnv* env) noexcept {
  constexpr const char* fallback =
      R"({"contract_version":2,"data":null,"error":{"code":"NATIVE_INTERNAL_ERROR","details":{"reason":"Habit JNI failure serialization failed"},"message":"Native internal error","retryable":false},"ok":false,"request_id":"00000000-0000-4000-8000-000000000000"})";
  if (env->ExceptionCheck()) env->ExceptionClear();
...
jstring call_habit_boundary(
  ...
  try {
    ...
    return failure_result(
        ...
        "Habit response is not valid UTF-8");
...
#define HABIT_V2_JNI(kotlin_name, cpp_name)                                                \
  extern "C" JNIEXPORT jstring JNICALL                                                    \
    ...
    return call_habit_boundary(                                                           \
        env, request_json, excellent_calendar::boundary::api::cpp_name);                  \
  }

}  // namespace

HABIT_V2_JNI(nativeCreateHabitV2, create_habit_v2)
HABIT_V2_JNI(nativeUpdateHabitV2, update_habit_v2)
HABIT_V2_JNI(nativeListHabitsV2, list_habits_v2)
HABIT_V2_JNI(nativeGetHabitDetailV2, get_habit_detail_v2)
HABIT_V2_JNI(nativeEndHabitV2, end_habit_v2)
HABIT_V2_JNI(nativeDeleteHabitV2, delete_habit_v2)
HABIT_V2_JNI(nativeCheckInHabitV2, check_in_habit_v2)
HABIT_V2_JNI(nativeClearHabitCheckInV2, clear_habit_check_in_v2)
HABIT_V2_JNI(nativeListHabitDailyStatusesV2, list_habit_daily_statuses_v2)
HABIT_V2_JNI(nativeSetHabitReminderV2, set_habit_reminder_v2)
HABIT_V2_JNI(nativeReconcileHabitRemindersV2, reconcile_habit_reminders_v2)
