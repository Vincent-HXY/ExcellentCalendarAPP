...
### 测试环节
受当前设备条件限制，项目暂不要求提供大规模真实手机或完整 Android 版本矩阵。

有真实手机可用时，优先在该手机上完成与本次开发相关的测试。相关验收场景通过后，即视为本次开发的设备验收通过，但不代表其他 Android 版本或厂商设备已经验证。

测试完成后，在 `test_note/` 中创建对应的 Markdown 记录，说明本次开发实际通过的设备型号、Android/API 版本和主要测试内容。该记录只需包含实际完成并通过的环境，无需逐项列出未验证版本；未被记录的版本不得视为已经通过。

没有真实手机可用时，保证 `EC_API36` 模拟器上的相关测试通过，即可视为本次开发的设备验收通过。

其他 Android 版本、厂商 ROM 和特殊设备环境的补充测试，由项目负责人后续根据 `test_note/` 中的已有记录统一安排。除非当前任务明确涉及特定系统版本、平台行为或已知兼容性缺陷，正常开发阶段无需扩展过大的设备测试矩阵，也不应因此阻塞常规开发进度。

当前阶段不为尚未确认的设备差异预先设计复杂的版本兼容分支或厂商专用绕过。发现实际兼容问题后，再根据明确的复现环境和测试证据进行针对性修复。

本策略用于限定当前开发阶段的设备验收范围，不用于宣称项目已经完成全部 Android 版本、设备型号或厂商 ROM 的兼容性验证。
...
- `test_note/`：仅记录本次实际完成并通过的设备或模拟器验收环境。
