...
- 导入仅在 publish-applied 后按 IMP-09 精确退休游客 live graph。提议采用 `guest_import_audit_anchors` 承接终态审计引用；非终态 Reminder/attempt 用 `source_migrated` 不可恢复终结。现有枚举/方法的变更必须另有兼容 Schema 后才能实施。
