#!/usr/bin/env python3
"""Bootstrap isolated Contract verification dependencies and run the R1 gate."""
...
VALIDATOR = CONTRACTS / "validate_anniversary_r1.py"
...
def main() -> int:
    ...
    environment["PYTHONPATH"] = str(cache) + (os.pathsep + existing_pythonpath if existing_pythonpath else "")
    return subprocess.run([sys.executable, str(VALIDATOR)], env=environment, check=False).returncode
